...
(function($){
	...
	$.fn.crossword = function(entryData) {
			...
			*/
			...
			this.after('<div id="puzzle-clues1" class="puzzle-clues"><div class=settings><label for="seeAnagram"><input id="seeAnagram" type="checkbox"/>Anagramme</label><input id="reset" type="button" value="Rejouer" /></div><h2>Horizontalement</h2><ul id="across"></ul><h2>Verticalement</h2><ul id="down"></ul></div>');
				...
				calcCoords: function() {
					...
					*/
					for (var i = 0, p = entryCount; i < p; ++i) {		
						...
						arrayAnswer = puzz.data[i].answer.split('');
						anagramAnswer = arrayAnswer.sort(() => Math.random() - 0.5);
						stringAnagram = anagramAnswer.join('');

						// while we're in here, add clues to DOM!
						$('#' + puzz.data[i].orientation).append('<li tabindex="1" class=definition-'+ puzz.data[i].id +' data-position="' + i + '">' + '<b>(' + puzz.data[i].position + ') - </b>' + puzz.data[i].clue + '<div class="anagram">' + stringAnagram +'</div></li>');
				...
				buildEntries: function() {
					...
					for (var x=1, p = entryCount; x <= p; ++x) {
						...
						for (var i=0; i < entries[x-1].length; ++i) {
							...
							if($(light).empty()){
									positionSolved = [x - positionOffset, x, x-1];
									$(light)
									.addClass('entry-' + (hasOffset ? x - positionOffset : x) + ' position-' + (x-1));
										$(light).append('<input maxlength="1" value="" type="text" tabindex="-1" />');

										if(solvedListId) {
											if(solvedArrayFiltered.some(item => parseInt(item) ===  x - 1) ){
												// $(light).find('input').attr('value', puzz.data[x-1].answer[i].split(''));
												$(light).find('input').addClass('done');
												$(light).addClass('solved');
											}
										}
							}
							if ($(light).hasClass('solved')) {
								if(solvedListId) {
									for (const element of solvedArrayFiltered) {
										$(light).find('input').attr('value', puzz.data[x-1].answer[i].split(''));
									}
					...
					for (var i=1, p = entryCount; i < p; ++i) {
						...
						if(!$('.entry-' + i +':eq(0) span').length){
							$groupedLights.eq(0)
								.append('<span class="first">' + puzz.data[i-1].position + '</span>');
						} else {
							$groupedLights.eq(0)
							.append('<span class="second">' + puzz.data[i-1].position + '</span>');		
						}
					}	
				...
				*/
				checkAnswer: function(e) {
					...
					if(valToCheck === currVal ){	
						...
						solvedId.push(puzz.data[activePosition].id);
						localStorage.setItem('puzzle1', solvedId);
						solved.push(valToCheck);
						solvedToggle = true;
						if($('.clue-done').length === entryCount) {
							$('#puzzle-wrapper1').append('<div class="puzzle-success">Felicitation</div>');
						}
						return;
					}
